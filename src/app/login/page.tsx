...
export default function LoginPage() {
  ...
  const [selectedRole, setSelectedRole] = useState<LoginRole>('agency');
  ...
  useEffect(() => {
    ...
    const typeFromUrl = params.get('type') as LoginRole | null;
    if (typeFromUrl && ['agency', 'organization'].includes(typeFromUrl)) {
      setSelectedRole(typeFromUrl);
    }
  }, []);
  ...
  const parseError = (errorMessage: string): ErrorState => {
    ...
    if (lowerError.includes('not associated with any organization') ||
      ...
      return { type: 'no_organization', message: t.auth.accountNotFoundMsg };
    }

    if (lowerError.includes('user not found') ||
        lowerError.includes('kullanıcı bulunamadı') ||
        lowerError.includes('kayıtlı değil')) {
      return { type: 'not_found', message: t.auth.userNotFoundMsg };
    }
    ...
    if (lowerError.includes('invalid otp') ||
      ...
      return { type: 'invalid_otp', message: t.auth.otpError };
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-50">
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-8 shadow-xl">
          <SprinterLoading message={mounted ? t.common.loading : 'Yükleniyor...'} size="lg" />
        </div>
      </div>
    );
  }
      ...
      >
        ...
        <div className="relative z-10 space-y-6">
          ...
          <div className="space-y-3 mt-6">
            <div className={selectedRole === 'agency' ? '' : 'hidden'}>
            ...
            <div className={selectedRole === 'organization' ? '' : 'hidden'}>
          ...
          <div className="w-full max-w-sm" suppressHydrationWarning>
            ...
            <div className={step === 'otp' ? '' : 'hidden'}>
              <div className="space-y-6">
                ...
                {/* OTP Error - always in DOM */}
                <div className={error && step === 'otp' ? '' : 'hidden'}>
                ...
                <div className="space-y-4">
                  ...
                  >
                    <span className="flex items-center">
                      <Loader2 className={`h-4 w-4 mr-2 animate-spin ${isSubmitting ? '' : 'hidden'}`} />
                      <CheckCircle2 className={`h-4 w-4 mr-2 ${isSubmitting ? 'hidden' : ''}`} />
                      <span>{isSubmitting ? t.auth.verifying : t.auth.login}</span>
                    </span>
                  </Button>
            ...
            <div className={step === 'email' ? '' : 'hidden'}>
              ...
              <form onSubmit={handleSubmit(onSubmitEmail)} className="space-y-4">
                {/* Email Error - always in DOM */}
                <div className={error && step === 'email' ? '' : 'hidden'}>
                  <ErrorMessage error={error} selectedRole={selectedRole} t={t} email={loginEmail} />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    {...register('email')}
                  />
                  <p className={`text-xs text-red-500 ${errors.email ? '' : 'hidden'}`}>{errors.email?.message || ' '}</p>
                </div>
                ...
                >
                  <span className="flex items-center">
                    <Loader2 className={`h-4 w-4 mr-2 animate-spin ${isSubmitting ? '' : 'hidden'}`} />
                    <Mail className={`h-4 w-4 mr-2 ${isSubmitting ? 'hidden' : ''}`} />
                    <span>{isSubmitting ? t.auth.sending : t.auth.sendOtp}</span>
                  </span>
                </Button>
