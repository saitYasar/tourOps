...
export default function AgencyClientLoginPage() {
  return (
    <Suspense fallback={
...
function AgencyClientLoginContent() {
  ...
  const [agency, setAgency] = useState<AgencyPublicDto | null>(null);
  ...
  // Email OTP state (shared between login-email and register)
  ...
  const handleSuccess = (data: any, isNewUser = false) => {
    ...
    // Full page reload so AuthProvider re-reads token from localStorage
  ...
  // Verify OTP (works for both email login and register)
  ...
  return (
    <div className="h-screen flex overflow-hidden">
      {/* Sol Panel - Travel Theme with Agency Info */}
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col justify-between p-8 xl:p-12 text-white w-full">
          ...
          <div className="relative z-10 space-y-6">
            {agency ? (
              <div>
                <div className="p-6 bg-white/15 backdrop-blur-md rounded-2xl border border-white/20 mb-6">
                  <div className="flex items-center gap-4 mb-4">
                    <div className="p-3 bg-white/20 rounded-xl">
                      <Building2 className="h-8 w-8" />
                    </div>
                    <div>
                      <h2 className="text-2xl xl:text-3xl font-bold">{agency.name}</h2>
                      {agency.legalName && agency.legalName !== agency.name && (
                        <p className="text-white/70 text-sm">{agency.legalName}</p>
                      )}
                    </div>
                  </div>
                  {agency.description && (
                    <p className="text-white/90 text-sm leading-relaxed mb-4">{agency.description}</p>
                  )}
                  <div className="space-y-2">
                    {agency.email && (
                      <div className="flex items-center gap-2 text-white/80 text-sm">
                        <Mail className="h-4 w-4" /><span>{agency.email}</span>
                      </div>
                    )}
                    {agency.phone && (
                      <div className="flex items-center gap-2 text-white/80 text-sm">
                        <Phone className="h-4 w-4" /><span>+{agency.phoneCountryCode} {agency.phone}</span>
                      </div>
                    )}
                    {agency.address && (
                      <div className="flex items-center gap-2 text-white/80 text-sm">
                        <MapPin className="h-4 w-4" /><span>{agency.address}</span>
                      </div>
                    )}
                  </div>
                </div>
                <p className="text-white/90 text-lg leading-relaxed max-w-md">{t.auth.customerLoginDesc}</p>
              </div>
            ) : (
              <div>
      ...
      <div className="flex-1 lg:w-1/2 flex flex-col bg-gradient-to-br from-rose-50 via-red-50 to-amber-50">
        <div className="flex-1 flex items-center justify-center p-4 sm:p-6 lg:p-8 overflow-y-auto">
          <div className="w-full max-w-sm">
            ...
            {agency && (
              <div className="lg:hidden mb-4 p-4 rounded-xl bg-gradient-to-r from-rose-50 to-amber-50 border border-rose-200">
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-gradient-to-r from-rose-500 to-red-500 rounded-lg text-white">
            ...
            <div className="space-y-5">
              {/* OTP Step (shared between email-login and register) */}
              {otpStep === 'otp' ? (
                <OtpVerifySection
                  email={email}
                  otp={otp}
                ...
                <>
                  ...
                  <div className="text-center">
                    ...
                    <h2 className="text-2xl font-bold text-slate-900 mb-1">
                      {agency ? agency.name : t.auth.clientLoginTitle}
                    </h2>
...
// OTP Verification Section (reused by email login & register)
function OtpVerifySection({
  ...
  onResend: () => void;
  onBack: () => void;
  t: ReturnType<typeof import('@/contexts/LanguageContext').useLanguage>['t'];
  buttonLabel: string;
}) {
  return (
    <div className="space-y-5">
