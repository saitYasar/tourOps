...
export default function AgencyTicketsPage() {
  ...
  const { data: ticketsRes, isLoading } = useQuery({
    queryKey: ['agency-tickets', statusFilter, categoryFilter, page, apiLang],
    queryFn: () =>
      ticketApi.agencyGetList(
        {
          page,
          limit,
  ...
  return (
    <div className="flex-1 overflow-auto bg-gray-50">
                  ...
                  {tickets.map((ticket) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell className="font-medium text-sm max-w-[250px] truncate">{ticket.subject}</TableCell>
      ...
      {/* Detail Dialog */}
      <AgencyTicketDetailDialog
        ticketId={detailTicketId}
        onClose={() => setDetailTicketId(null)}
        apiLang={apiLang}
        onStatusChange={() => queryClient.invalidateQueries({ queryKey: ['agency-tickets'] })}
      />
    </div>
  );
}
...
// ============================================
// Agency Ticket Detail Dialog (with status change + messaging)
// ============================================

function AgencyTicketDetailDialog({
  ticketId,
  onClose,
  apiLang,
  onStatusChange,
  ...
  const { data: ticketRes, isLoading } = useQuery({
    ...
    queryFn: () => ticketApi.agencyGetById(ticketId!, 1, 200, apiLang),
    enabled: !!ticketId,
  });
  ...
  const markReadMutation = useMutation({
    mutationFn: () => ticketApi.agencyMarkRead(ticketId!, apiLang),
  });
  const markedRef = useRef<number | null>(null);
  if (ticketId && ticket && markedRef.current !== ticketId) {
    markedRef.current = ticketId;
    const hasUnread = messages_list.some((m) => m.senderType === 'client' && !m.isRead);
    if (hasUnread) markReadMutation.mutate();
  }

  // Send message
  const sendMutation = useMutation({
    mutationFn: () => ticketApi.agencySendMessage(ticketId!, message.trim(), files.length > 0 ? files : undefined, apiLang),
    onSuccess: (res) => {
      if (res.success) {
        setMessage('');
        setFiles([]);
        queryClient.invalidateQueries({ queryKey: ['agency-ticket-detail', ticketId] });
      } else {
  ...
  const statusMutation = useMutation({
    mutationFn: (status: TicketStatus) => ticketApi.agencyUpdateStatus(ticketId!, status, apiLang),
    onSuccess: (res) => {
      if (res.success) {
  ...
  const senderLabel = (type: string) => {
    ...
    if (type === 'agency_user') return tt.you;
    return tt.admin;
  };
        ...
        <div className="flex-1 overflow-y-auto px-5 py-3 space-y-3 min-h-0">
          {isLoading ? (
            ...
            messages_list.map((msg) => {
              const isOwn = msg.senderType === 'agency_user';
              return (
                <div key={msg.id} className={`flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[75%] rounded-2xl px-3.5 py-2 ${isOwn ? 'bg-slate-800 text-white rounded-br-md' : 'bg-slate-100 text-slate-800 rounded-bl-md'}`}>
                    {!isOwn && (
                      ...
                      <div className="mt-1.5 space-y-1">
                        {msg.attachments.map((att, i) => (
                          <a
                            ...
                            className={`flex items-center gap-1 text-xs underline ${isOwn ? 'text-slate-300' : 'text-blue-600'}`}
                          >
                    ...
                    <p className={`text-[9px] mt-1 ${isOwn ? 'text-slate-400' : 'text-slate-400'}`}>
        ...
        {isClosed ? (
          ...
          <div className="px-5 py-3 border-t bg-white shrink-0">
            ...
            <div className="flex items-end gap-2">
              ...
              <Button
                size="sm"
                className="bg-slate-800 hover:bg-slate-900 text-white h-10 w-10 p-0 shrink-0"
